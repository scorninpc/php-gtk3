...
bool phpgtk_check_parameter(Php::Parameters &parameters, int param,
                            Php::Type expected_type, bool required,
  ...
  if (required) {
    ...
    if (param_count < param) {
      throw Php::Exception(std::string("Missing required parameter ") +
                           std::to_string(param));
    }
    ...
    // Text Object type
    if ((expected_type == Php::Type::Object) &&
        (!Php::is_a(parameters[param - 1], object_type))) {
      throw Php::Exception(phpgtk_wrong_type_message(
          param, parameters[param - 1].type(), expected_type));
    }
    ...
    if ((expected_type == Php::Type::Object) &&
        (!Php::is_a(parameters[param - 1], object_type))) {
      Php::warning << "Invalid type for optional parameter " << param
                   << std::flush;
      return false;
    }
    ...
    // General type check
    if (parameters[param - 1].type() != expected_type) {
      Php::warning << phpgtk_wrong_type_message(
                          param, parameters[param - 1].type(), expected_type)
                   << std::flush;
      return false;
    }
 ...
 *
 */
char *phpgtk_wrong_type_message(int param, Php::Type type_passed,
                                Php::Type type_expected) {
  char *buffer = nullptr;
  int len = 0;

  // Get len of string
  len = snprintf(nullptr, 0, "Expected parameter %d to be an %s, %s given",
                 param, phpgtk_type_to_string(type_expected).c_str(),
                 phpgtk_type_to_string(type_passed).c_str());
  buffer = (char *)malloc((len + 1) * sizeof(char));

  // Save into buffer
  snprintf(buffer, len + 1, "Expected parameter %d to be an %s, %s given",
           param, phpgtk_type_to_string(type_expected).c_str(),
           phpgtk_type_to_string(type_passed).c_str());
...
Php::Value cobject_to_phpobject(gpointer *cobject) {
  ...
  return Php::Object(g_type_name(G_TYPE_FROM_INSTANCE((gpointer *)cobject)),
                     return_parsed);
}
...
void generic_callback(gpointer *self, ...) {
  /**
   *  This will loop all vargs after *self, max to 5 times, casting to
   * generic_st_callback if the generic_st_callback.callback_name is a php land
   * user function, this will be callable, so we know that this param is the
   * userdata
   */
  struct generic_st_callback *callback_object = nullptr;
  ...
  for (int i = 0; i < 5; i++) {
    callback_object =
        (struct generic_st_callback *)va_arg(ap, generic_st_callback *);
