 ...
 */
GIcon *GIcon_::get_instance() const { return instance; }
...
void GIcon_::set_instance(GIcon *passed_gicon, bool add_reference) {
  ...
  // Pass add_reference=false when transferring ownership from a newly created
  // object
...
Php::Value GIcon_::new_from_pixbuf(Php::Parameters &parameters) {
  ...
  if (parameters.empty()) {
    throw Php::Exception(
        "GIcon::new_from_pixbuf() requires 1 parameter: GdkPixbuf object");
  }
  ...
  // This is necessary because GIcon is an interface and we need a concrete
  // implementation
  ...
  // Suppress the warning that occurs when the GIcon is later used with
  // GtkStatusIcon.
  {
  ...
  // Pass false to set_instance since we're transferring the reference from
  // g_bytes_icon_new
...
Php::Value GIcon_::new_from_stock(Php::Parameters &parameters) {
  ...
  if (parameters.empty()) {
    throw Php::Exception(
        "GIcon::new_from_stock() requires 1 parameter: stock_id");
  }

  std::string c_stock_id = parameters[0];
  const gchar *stock_id = c_stock_id.c_str();

  // GTK may call gtk_widget_get_scale_factor when this themed icon is later
  // used, even though g_themed_icon_new itself doesn't involve scale factor
  // queries. Suppress the warning that occurs when the GIcon is used with
  // GtkStatusIcon.
  GtkLogSuppressor suppressor;
