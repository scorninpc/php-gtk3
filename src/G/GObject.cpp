...
void GObject_::__clone() const {
  ...
  // Get len of string
  len =
      snprintf(nullptr, 0, "Trying to clone an uncloneable object of class %s",
               gtype_name.c_str());
  buffer = (char *)malloc((len + 1) * sizeof(char));

  // Save into buffer
  snprintf(buffer, len + 1, "Trying to clone an uncloneable object of class %s",
           gtype_name.c_str());
 ...
 */
gpointer *GObject_::get_instance() const { return instance; }
...
Php::Value GObject_::connect_internal(Php::Parameters &parameters, bool after) {
  ...
  // This prevents critical errors when calling widget-specific methods on
  // non-widget GObjects
  const gchar *type_name = ((instance != nullptr) && G_IS_OBJECT(instance))
  ...
  if (G_IS_OBJECT(instance)) {
    g_signal_query(g_signal_lookup(callback_event, G_OBJECT_TYPE(instance)),
                   &signal_info);
  }

  if (G_IS_OBJECT_CLASS(instance)) {
    g_signal_query(
        g_signal_lookup(callback_event, G_OBJECT_CLASS_TYPE(instance)),
        &signal_info);
  }
  ...
  // closure = g_cclosure_new_swap (G_CALLBACK (connect_callback),
  // callback_object, (GClosureNotify)destroy_notify);

  closure = g_cclosure_new_swap(G_CALLBACK(connect_callback), callback_object,
                                nullptr);
  int ret = g_signal_connect_closure(instance, callback_event, closure,
                                     static_cast<gboolean>(after));
...
bool GObject_::connect_callback(gpointer user_data, ...) {
  ...
  for (int i = 0; i < param_count; i++) {
    ...
    case G_TYPE_FLAGS:
      ...
      // Flags are passed as unsigned integers (promoted to unsigned int in
      // varargs)
      internal_parameters[i + 1] = (int)va_arg(ap, guint);
      break;
    ...
    case G_TYPE_OBJECT: {
      ...
      const gchar *param_type_name =
          g_type_name(callback_object->param_types[i]);
      std::string type_name =
          (param_type_name != nullptr) ? param_type_name : "GObject";
  ...
  // This ensures exceptions work correctly even with Xdebug exception
  // breakpoints enabled
  try {
    Php::Value ret =
    ...
    // This allows PHP try-catch blocks to catch it and Xdebug to track it
    // correctly
    throw;
  }
  ...
  // Php::call("var_dump",
  // G_TYPE_IS_FUNDAMENTAL(G_TYPE_FROM_CLASS(user_param)));
  ...
  // // Php::Value custom_parameters = Php::call("array_slice", callback_params,
  // 2, callback_params.size());
  // // Php::Value php_callback_param = Php::call("array_merge",
  // internal_parameters, custom_parameters);

  // // Call php function with parameters
  // Php::Value ret = Php::call("call_user_func_array",
  // callback_object->callback_name, internal_parameters);
 ...
 */
void GObject_::handler_disconnect(Php::Parameters &parameters) const {
...
Php::Value GObject_::is_connected(Php::Parameters &parameters) const {
  Php::Value callback_handle = parameters[0];
  ...
  bool ret = g_signal_handler_is_connected(instance, (int)callback_handle) != 0;

  return ret;
}

Php::Value GObject_::get_property(Php::Parameters &parameters) const {
...
void GObject_::set_property(Php::Parameters &parameters) const {
  std::string s_property_name = parameters[0];
  ...
  if (strcmp(property_name, "model") == 0) {
    ...
    g_object_set(G_OBJECT(instance), "model", o_object->get_model(),
                 (char *)nullptr);
  } else {
    // get the property spec
    GParamSpec *prop = g_object_class_find_property(
        G_OBJECT_GET_CLASS(instance), property_name);
    if (prop == nullptr) {
      std::string error;
      throw Php::Exception(error + "there is no property " + property_name +
                           " on object " +
    ...
    GValue value =
        phpgtk_get_gvalue(parameters[1], G_TYPE_FUNDAMENTAL(prop->value_type));
...
void GObject_::signal_handler_block(Php::Parameters &parameters) const {
...
void GObject_::signal_handler_unblock(Php::Parameters &parameters) const {
  double p_handler_id = parameters[0];
  gulong handler_id = (gulong)p_handler_id;
...
Php::Value GObject_::get_data(Php::Parameters &parameters) const {
...
void GObject_::set_data(Php::Parameters &parameters) const {
  std::string s_key = parameters[0];
  gchar *key = (gchar *)s_key.c_str();
