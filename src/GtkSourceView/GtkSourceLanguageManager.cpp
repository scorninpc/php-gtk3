...
Php::Value
GtkSourceLanguageManager_::get_language(Php::Parameters &parameters) {
  std::string param_id = parameters[0];
  gchar *id = (gchar *)param_id.c_str();

  GtkSourceLanguage *ret = gtk_source_language_manager_get_language(
      GTK_SOURCE_LANGUAGE_MANAGER(instance), id);
...
Php::Value GtkSourceLanguageManager_::get_search_path() {
  ...
  const gchar *const *paths = gtk_source_language_manager_get_search_path(
      GTK_SOURCE_LANGUAGE_MANAGER(instance));
...
void GtkSourceLanguageManager_::set_search_path(Php::Parameters &parameters) {
  ...
  gtk_source_language_manager_set_search_path(
      GTK_SOURCE_LANGUAGE_MANAGER(instance), dirs);
...
Php::Value
GtkSourceLanguageManager_::guess_language(Php::Parameters &parameters) {
