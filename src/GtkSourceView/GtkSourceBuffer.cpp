...
void GtkSourceBuffer_::set_language(Php::Parameters &parameters) {
  GtkSourceLanguage *language = nullptr;
  if (!parameters.empty()) {
    Php::Value object_language = parameters[0];
    GtkSourceLanguage_ *phpgtk_language =
        (GtkSourceLanguage_ *)object_language.implementation();
...
Php::Value GtkSourceBuffer_::new_with_language(Php::Parameters &parameters) {
  ...
  GtkSourceLanguage *language =
      GTK_SOURCE_LANGUAGE(phpgtk_language->get_instance());
...
void GtkSourceBuffer_::change_case(Php::Parameters &parameters) {
  ...
  GtkSourceChangeCaseType case_type =
      (GtkSourceChangeCaseType)(int)parameters[0];
  ...
  // call gtk function
  gtk_source_buffer_change_case(GTK_SOURCE_BUFFER(instance), case_type, &start,
                                &end);
}
