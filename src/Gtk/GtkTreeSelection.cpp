...
Php::Value GtkTreeSelection_::get_mode() {
  GtkSelectionMode ret =
      gtk_tree_selection_get_mode(GTK_TREE_SELECTION(instance));
...
void GtkTreeSelection_::set_select_function(Php::Parameters &parameters) {
  ...
  // 	GtkTreeIter_ *phpgtk_iter = (GtkTreeIter_
  // *)object_iter.implementation(); 	iter =
  // GTK_WIDGET(phpgtk_iter->get_instance());
  // }

  // gtk_tree_selection_set_select_function (GTK_TREE_SELECTION(instance),
  // iter);

  throw Php::Exception(
      "GtkTreeSelection_->set_select_function() not implemented yet");
}
...
void GtkTreeSelection_::get_select_function(Php::Parameters &parameters) {
  ...
  // 	GtkTreeIter_ *phpgtk_parent = (GtkTreeIter_
  ...
  // gtk_tree_selection_get_select_function (GTK_TREE_SELECTION(instance),
  // parent, position);

  throw Php::Exception(
      "GtkTreeSelection_->get_select_function() not implemented yet");
}
...
Php::Value GtkTreeSelection_::get_user_data(Php::Parameters &parameters) {
  ...
  // 	GtkTreeIter_ *phpgtk_sibling = (GtkTreeIter_
  // *)object_sibling.implementation(); 	sibling =
  // GTK_WIDGET(phpgtk_sibling->get_instance());
  // }

  // gpointer ret = gtk_tree_selection_get_user_data
  // (GTK_TREE_SELECTION(instance), parent, sibling);

  // return ret;

  throw Php::Exception(
      "GtkTreeSelection_->get_user_data() not implemented yet");
}

Php::Value GtkTreeSelection_::get_tree_view() {
  GtkTreeView *ret =
      gtk_tree_selection_get_tree_view(GTK_TREE_SELECTION(instance));
...
Php::Value GtkTreeSelection_::get_selected(Php::Parameters &parameters) {
  ...
  bool ret = gtk_tree_selection_get_selected(GTK_TREE_SELECTION(instance),
                                             &model, &iter) != 0;
  if (!ret) {
    return false;
  }
...
void GtkTreeSelection_::selected_foreach(Php::Parameters &parameters) {
  ...
  // GtkTreeIter *ret = gtk_tree_selection_selected_foreach
  // (GTK_TREE_SELECTION(instance), parent, position);
   ...
   * on the caller
   *
   * - create a struct with CALLBACK_SPEC, SELF_OBJECT, PHP_FUNCTION_TO_CALL,
   * USER_PARAMETERS
  ...
  // mount the function params, like showed ini
  ...
  callback_object->param_types =
      (GType *)malloc(sizeof(GType) * callback_object->n_params);
  ...
  // GClosure  *closure;
  // closure = g_cclosure_new_swap (G_CALLBACK (generic_callback),
  // callback_object, NULL);

  // // test

  gtk_tree_selection_selected_foreach(
      GTK_TREE_SELECTION(instance),
      (GtkTreeSelectionForeachFunc)generic_callback, callback_object);
...
Php::Value GtkTreeSelection_::get_selected_rows() {
  ...
  GList *ret = gtk_tree_selection_get_selected_rows(
      GTK_TREE_SELECTION(instance), &model);
...
Php::Value GtkTreeSelection_::count_selected_rows() {
  gint ret =
      gtk_tree_selection_count_selected_rows(GTK_TREE_SELECTION(instance));
...
Php::Value GtkTreeSelection_::path_is_selected(Php::Parameters &parameters) {
  ...
  bool ret = gtk_tree_selection_path_is_selected(GTK_TREE_SELECTION(instance),
                                                 path) != 0;
...
Php::Value GtkTreeSelection_::iter_is_selected(Php::Parameters &parameters) {
  ...
  bool ret = gtk_tree_selection_iter_is_selected(GTK_TREE_SELECTION(instance),
                                                 &iter) != 0;
...
void GtkTreeSelection_::unselect_range(Php::Parameters &parameters) {
  ...
  GtkTreePath *start_path =
      gtk_tree_path_new_from_string(param_start_path.c_str());

  std::string param_end_path = parameters[1];
  GtkTreePath *end_path = gtk_tree_path_new_from_string(param_end_path.c_str());

  gtk_tree_selection_unselect_range(GTK_TREE_SELECTION(instance), start_path,
                                    end_path);
}
void GtkTreeSelection_::select_range(Php::Parameters &parameters) {
  ...
  gtk_tree_selection_select_range(GTK_TREE_SELECTION(instance), start_path,
                                  end_path);
}
