...
Php::Value GtkExpander_::get_expanded() {
  bool ret = gtk_expander_get_expanded(GTK_EXPANDER(instance)) != 0;
...
Php::Value GtkExpander_::get_use_underline() {
  bool ret = gtk_expander_get_use_underline(GTK_EXPANDER(instance)) != 0;
...
Php::Value GtkExpander_::get_use_markup() {
  bool ret = gtk_expander_get_use_markup(GTK_EXPANDER(instance)) != 0;

  return ret;
}

void GtkExpander_::set_label_widget(Php::Parameters &parameters) {
  GtkWidget *label_widget = nullptr;
  if (!parameters.empty()) {
    Php::Value object_label_widget = parameters[0];
    GtkWidget_ *phpgtk_label_widget =
        (GtkWidget_ *)object_label_widget.implementation();
...
Php::Value GtkExpander_::get_label_widget() {
  gpointer *ret =
      (gpointer *)gtk_expander_get_label_widget(GTK_EXPANDER(instance));
...
Php::Value GtkExpander_::get_label_fill() {
  bool ret = gtk_expander_get_label_fill(GTK_EXPANDER(instance)) != 0;
...
Php::Value GtkExpander_::get_resize_toplevel() {
  bool ret = gtk_expander_get_resize_toplevel(GTK_EXPANDER(instance)) != 0;
