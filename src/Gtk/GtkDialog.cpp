...
void GtkDialog_::__construct() { instance = (gpointer *)gtk_dialog_new(); }
...
Php::Value GtkDialog_::new_with_buttons(Php::Parameters &parameters) {
  ...
  GtkWidget *instance_dialog = gtk_dialog_new_with_buttons(
      title, parent, flags, arr[0], (int)arr[1], NULL);

  // Add buttons
  for (int index = 2; index < (int)arr.size(); index += 2) {
    gtk_dialog_add_button(GTK_DIALOG(instance_dialog), arr[index],
                          (int)arr[index + 1]);
  }
...
Php::Value GtkDialog_::get_widget_for_response(Php::Parameters &parameters) {
  ...
  gpointer *ret = (gpointer *)gtk_dialog_get_widget_for_response(
      GTK_DIALOG(instance), response_id);
