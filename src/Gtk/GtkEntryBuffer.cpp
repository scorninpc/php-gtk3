 ...
 */
GtkEntryBuffer *GtkEntryBuffer_::get_buffer() const { return buffer; }
...
Php::Value GtkEntryBuffer_::insert_text(Php::Parameters &parameters) {
  ...
  int length = (int)gtk_entry_buffer_insert_text(GTK_ENTRY_BUFFER(instance),
                                                 position, parameters[1], -1);
  return length;
}
