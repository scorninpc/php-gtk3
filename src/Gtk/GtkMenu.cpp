...
void GtkMenu_::popup_callback_destroy(gpointer data) {
  struct st_popup_callback *callback_object =
      static_cast<struct st_popup_callback *>(data);
 ...
 */
void GtkMenu_::popup_position_callback(GtkMenu *menu, gint *x, gint *y,
                                       gboolean *push_in, gpointer user_data) {
  struct st_popup_callback *callback_object =
      static_cast<struct st_popup_callback *>(user_data);
  ...
  if (callback_object->use_gtk_status_icon_position) {
    ...
    if (!callback_object->user_data.isNull() &&
        callback_object->user_data.isObject()) {
      ...
      if (status_icon_wrapper != nullptr) {
        GtkStatusIcon *status_icon =
            GTK_STATUS_ICON(status_icon_wrapper->get_instance());
        gtk_status_icon_position_menu(menu, x, y, push_in, status_icon);
        return;
      }
    }
    // Fall through to default positioning if status icon is invalid
    Php::warning << "GtkMenu::popup: status icon position_menu requires a "
                    "valid GtkStatusIcon object"
                 << std::flush;
    *x = 0;
    *y = 0;
    *push_in = TRUE;
  ...
  try {
    ...
    Php::Value result = Php::call("call_user_func_array",
                                  callback_object->callback_function, params);

    // The callback should return an array with 'x', 'y', and optionally
    // 'push_in'
    if (result.isArray()) {
      ...
      Php::warning << "GtkMenu::popup positioning callback did not return an "
                      "array; using default position (0, 0)"
                   << std::flush;
    }
  } catch (const Php::Exception &e) {
    // Log the exception to help diagnose callback failures, but do not let it
    // cross the C callback boundary (GtkMenuPositionFunc).
    Php::warning << "GtkMenu::popup positioning callback failed: " << e.what()
                 << std::flush;
...
void GtkMenu_::__construct() { instance = (gpointer *)gtk_menu_new(); }
...
void GtkMenu_::set_screen(Php::Parameters &parameters) {
  ...
  // 	GdkScreen_ *phpgtk_screen = (GdkScreen_
  // *)object_screen.implementation(); 	screen =
  // GTK_WIDGET(phpgtk_screen->get_instance());
  // }
...
void GtkMenu_::attach(Php::Parameters &parameters) {
  ...
  gtk_menu_attach(GTK_MENU(instance), child, left_attach, right_attach,
...
void GtkMenu_::popup_at_rect(Php::Parameters &parameters) {
  ...
  // 	GdkWindow_ *phpgtk_rect_window = (GdkWindow_
  // *)object_rect_window.implementation(); 	rect_window =
  // GTK_WIDGET(phpgtk_rect_window->get_instance());
  // }

  // GdkRectangle *rect;
  // if(parameters.size() > 1) {
  // 	Php::Value object_rect = parameters[1];
  // 	GdkRectangle_ *phpgtk_rect = (GdkRectangle_
  // *)object_rect.implementation(); 	rect =
  // GTK_WIDGET(phpgtk_rect->get_instance());
  // }

  // GdkGravity *rect_anchor;
  // if(parameters.size() > 2) {
  // 	Php::Value object_rect_anchor = parameters[2];
  // 	GdkGravity_ *phpgtk_rect_anchor = (GdkGravity_
  // *)object_rect_anchor.implementation(); 	rect_anchor =
  // GTK_WIDGET(phpgtk_rect_anchor->get_instance());
  // }

  // GdkGravity *menu_anchor;
  // if(parameters.size() > 3) {
  // 	Php::Value object_menu_anchor = parameters[3];
  // 	GdkGravity_ *phpgtk_menu_anchor = (GdkGravity_
  // *)object_menu_anchor.implementation(); 	menu_anchor =
  // GTK_WIDGET(phpgtk_menu_anchor->get_instance());
  // }

  // GdkEvent *trigger_event;
  // if(parameters.size() > 4) {
  // 	Php::Value object_trigger_event = parameters[4];
  // 	GdkEvent_ *phpgtk_trigger_event = (GdkEvent_
  // *)object_trigger_event.implementation(); 	trigger_event =
  // GTK_WIDGET(phpgtk_trigger_event->get_instance());
  // }

  // gtk_menu_popup_at_rect (GTK_MENU(instance), rect_window, rect, rect_anchor,
  // menu_anchor, trigger_event);
...
void GtkMenu_::popup_at_widget(Php::Parameters &parameters) {
  ...
  // 	GdkGravity_ *phpgtk_widget_anchor = (GdkGravity_
  // *)object_widget_anchor.implementation(); 	widget_anchor =
  // GTK_WIDGET(phpgtk_widget_anchor->get_instance());
  // }
  ...
  // gtk_menu_popup_at_widget (GTK_MENU(instance), widget, widget_anchor,
  // menu_anchor, trigger_event);
...
void GtkMenu_::popup_at_pointer(Php::Parameters &parameters) {
  ...
  GdkEvent_ *phpgtk_trigger_event =
      (GdkEvent_ *)object_trigger_event.implementation();
...
void GtkMenu_::popup(Php::Parameters &parameters) {
  ...
  if (!parameters.empty() && !parameters[0].isNull()) {
    if (!parameters[0].isObject()) {
      throw Php::Exception("GtkMenu::popup parameter 0 (parent_menu_shell) "
                           "expects a GtkWidget or null");
    }
    ...
    if (phpgtk_parent_menu_shell == nullptr) {
      throw Php::Exception("GtkMenu::popup parameter 0 (parent_menu_shell) "
                           "must be a GtkWidget instance");
    }
  ...
  if (parameters.size() > 1 && !parameters[1].isNull()) {
    if (!parameters[1].isObject()) {
      throw Php::Exception("GtkMenu::popup parameter 1 (parent_menu_item) "
                           "expects a GtkWidget or null");
    }
    ...
    if (phpgtk_parent_menu_item == nullptr) {
      throw Php::Exception("GtkMenu::popup parameter 1 (parent_menu_item) must "
                           "be a GtkWidget instance");
    }
  ...
  if (parameters.size() > 2 && !parameters[2].isNull()) {
    // Accept only valid callables (arrays are allowed only when they are
    // callable, e.g. [$object, 'methodName'])
    if (!parameters[2].isCallable()) {
      throw Php::Exception(
          "GtkMenu::popup parameter 2 (func) must be a valid callable or null. "
          "Use [$statusIconObject, 'position_menu'] for instance methods, not "
    ...
    // Check if user_data is provided (parameter 3) - only if not using special
    // gtk function
    if (!callback_object->use_gtk_status_icon_position &&
        parameters.size() > 3 && !parameters[3].isNull()) {
  ...
  // Call gtk_menu_popup
  // Note: gtk_menu_popup is deprecated since GTK 3.22, but still available in
  // GTK 3.24 The callback data is managed by GObject and will be freed when no
  // longer needed
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  gtk_menu_popup(GTK_MENU(instance), parent_menu_shell, parent_menu_item, func,
  ...
  g_object_set_data_full(
      G_OBJECT(instance), "php-gtk-popup-callback", callback_object,
      callback_object != nullptr ? popup_callback_destroy : nullptr);
}
...
void GtkMenu_::place_on_monitor(Php::Parameters &parameters) {
  ...
  // 	GdkMonitor_ *phpgtk_monitor = (GdkMonitor_
  // *)object_monitor.implementation(); 	monitor =
  // GTK_WIDGET(phpgtk_monitor->get_instance());
  // }
...
Php::Value GtkMenu_::get_reserve_toggle_size() {
  bool ret = gtk_menu_get_reserve_toggle_size(GTK_MENU(instance)) != 0;

  return ret;
}

void GtkMenu_::popdown() { gtk_menu_popdown(GTK_MENU(instance)); }

void GtkMenu_::reposition() { gtk_menu_reposition(GTK_MENU(instance)); }
...
void GtkMenu_::attach_to_widget(Php::Parameters &parameters) {
  ...
  // 	GtkWidget_ *phpgtk_attach_widget = (GtkWidget_
  // *)object_attach_widget.implementation(); 	attach_widget =
  // GTK_WIDGET(phpgtk_attach_widget->get_instance());
  // }
...
void GtkMenu_::detach() { gtk_menu_detach(GTK_MENU(instance)); }
