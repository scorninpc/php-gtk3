...
Php::Value GtkClipboard_::get_for_display(Php::Parameters &parameters) {
  ...
  // 	GdkDisplay_ *phpgtk_display = (GdkDisplay_
  // *)object_display.implementation(); 	display =
  // GTK_WIDGET(phpgtk_display->get_instance());
  // }

  // int int_selection = (int)parameters[1];
  // GdkSelection selection = (GdkSelection)int_selection;

  // GtkClipboard *ret = gtk_clipboard_get_for_display (GTK_CLIPBOARD(instance),
  // display, selection);
...
Php::Value GtkClipboard_::set_with_data(Php::Parameters &parameters) {
  ...
  // 	GtkTargetEntry_ *phpgtk_targets = (GtkTargetEntry_
  // *)object_targets.implementation(); 	targets =
  // GTK_WIDGET(phpgtk_targets->get_instance());
  // }

  // guint n_targets = (guint)parameters[1];

  // gboolean ret = gtk_clipboard_set_with_data (GTK_CLIPBOARD(instance),
  // targets, n_targets);
...
Php::Value GtkClipboard_::set_with_owner(Php::Parameters &parameters) {
  ...
  // gboolean ret = gtk_clipboard_set_with_owner (GTK_CLIPBOARD(instance),
  // targets, n_targets);
...
void GtkClipboard_::request_contents(Php::Parameters &parameters) {
  ...
  // gtk_clipboard_request_contents (GTK_CLIPBOARD(instance), user_function,
  // user_data);
...
void GtkClipboard_::request_text_callback(GtkClipboard *clipboard,
                                          const gchar *clipboard_text,
                                          gpointer user_data) {
  // Return to st_callback
  struct st_request_callback *callback_object =
      (struct st_request_callback *)user_data;
...
void GtkClipboard_::request_image(Php::Parameters &parameters) {
  ...
  // gtk_clipboard_request_image (GTK_CLIPBOARD(instance), user_function,
  // user_data);
...
void GtkClipboard_::request_targets(Php::Parameters &parameters) {
  ...
  // gtk_clipboard_request_targets (GTK_CLIPBOARD(instance), user_function,
  // user_data);
...
void GtkClipboard_::request_rich_text(Php::Parameters &parameters) {
  ...
  // gtk_clipboard_request_rich_text (GTK_CLIPBOARD(instance), user_function,
  // user_data);
...
void GtkClipboard_::request_uris(Php::Parameters &parameters) {
  ...
  // gtk_clipboard_request_uris (GTK_CLIPBOARD(instance), user_function,
  // user_data);
...
Php::Value GtkClipboard_::wait_for_contents(Php::Parameters &parameters) {
  ...
  // GtkSelectionData *ret = gtk_clipboard_wait_for_contents
  // (GTK_CLIPBOARD(instance), target);
...
Php::Value GtkClipboard_::wait_for_rich_text(Php::Parameters &parameters) {
  ...
  // 	GtkTextBuffer_ *phpgtk_buffer = (GtkTextBuffer_
  // *)object_buffer.implementation(); 	buffer =
  // GTK_TEXT_BUFFER(phpgtk_buffer->get_instance());
  // }

  // std::string s_target = parameters[1];
  // gchar *target = (gchar *)s_target.c_str();

  // guint8 ret = gtk_clipboard_wait_for_rich_text (GTK_CLIPBOARD(instance),
  // buffer, target);
...
Php::Value GtkClipboard_::wait_is_text_available() {
  bool ret = gtk_clipboard_wait_is_text_available(GTK_CLIPBOARD(instance)) != 0;
...
Php::Value GtkClipboard_::wait_is_image_available(Php::Parameters &parameters) {
  ...
  bool ret = gtk_clipboard_wait_is_rich_text_available(GTK_CLIPBOARD(instance),
                                                       buffer) != 0;
...
Php::Value GtkClipboard_::wait_is_uris_available() {
  bool ret = gtk_clipboard_wait_is_uris_available(GTK_CLIPBOARD(instance)) != 0;

  return ret;
}

Php::Value
GtkClipboard_::wait_is_target_available(Php::Parameters &parameters) {
  // std::string s_target = parameters[0];
  // gchar *target = (gchar *)s_target.c_str();

  // gboolean ret = gtk_clipboard_wait_is_target_available
  // (GTK_CLIPBOARD(instance), target);
