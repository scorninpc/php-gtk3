...
void GtkFlowBox_::selected_foreach(Php::Parameters &parameters) {
  ...
  // *)object_func.implementation(); 	func = GTK_WIDGET(phpgtk_func->get_instance());
  // }
...
void GtkFlowBox_::bind_model(Php::Parameters &parameters) {
  ...
  // 	GtkFlowBoxCreateWidgetFunc_ *phpgtk_create_widget_func = (GtkFlowBoxCreateWidgetFunc_
  // *)object_create_widget_func.implementation(); 	create_widget_func =
  // GTK_WIDGET(phpgtk_create_widget_func->get_instance());
  // }

  // gpointer user_data = (gpointer)parameters[2];

  // gtk_flow_box_bind_model (GTK_FLOW_BOX(instance), model, create_widget_func, user_data,
  // user_data_free_func);
