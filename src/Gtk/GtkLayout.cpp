...
void GtkLayout_::__construct(Php::Parameters &parameters) {
  GtkAdjustment *hadjustment = nullptr;
  if (!parameters.empty()) {
    Php::Value object_hadjustment = parameters[0];
    GtkAdjustment_ *phpgtk_hadjustment =
        (GtkAdjustment_ *)object_hadjustment.implementation();
    hadjustment = GTK_ADJUSTMENT(phpgtk_hadjustment->get_instance());
  }

  GtkAdjustment *vadjustment = nullptr;
  if (parameters.size() > 1) {
    Php::Value object_vadjustment = parameters[1];
    GtkAdjustment_ *phpgtk_vadjustment =
        (GtkAdjustment_ *)object_vadjustment.implementation();
...
void GtkLayout_::put(Php::Parameters &parameters) {
  GtkWidget *child_widget = nullptr;
  if (!parameters.empty()) {
    Php::Value object_child_widget = parameters[0];
    GtkWidget_ *phpgtk_child_widget =
        (GtkWidget_ *)object_child_widget.implementation();
