...
void GtkListStore_::set(Php::Parameters &parameters) {
  ...
  // // Get column type
  // GType type_column = gtk_tree_model_get_column_type(GTK_TREE_MODEL(model),
  // column);
...
void GtkListStore_::set_valist(Php::Parameters &parameters) {
  ...
  // *)object_iter.implementation(); 	iter = phpgtk_iter->get_instance();
  // }
...
void GtkListStore_::set_value(Php::Parameters &parameters) {
  ...
  // Get column type
  GType type_column =
      gtk_tree_model_get_column_type(GTK_TREE_MODEL(model), column);
...
void GtkListStore_::set_valuesv(Php::Parameters &parameters) {
  ...
  // gtk_list_store_set_valuesv (GTK_LIST_STORE(model), iter, columns, values,
  // n_values);
...
Php::Value GtkListStore_::remove(Php::Parameters &parameters) {
  ...
  bool ret = gtk_list_store_remove(GTK_LIST_STORE(model), &iter) != 0;
...
Php::Value GtkListStore_::insert_before(Php::Parameters &parameters) {
  ...
  GtkTreeIter_ *phpgtk_sibling =
      (GtkTreeIter_ *)object_sibling.implementation();
...
Php::Value GtkListStore_::prepend(Php::Parameters &parameters) {
  ...
  for (int index = 0; index < (int)arr.size(); index++) {
    ...
    // Get column type
    GType type_column =
        gtk_tree_model_get_column_type(GTK_TREE_MODEL(model), index);
...
void GtkListStore_::clear() { gtk_list_store_clear(GTK_LIST_STORE(model)); }
...
Php::Value GtkListStore_::iter_is_valid(Php::Parameters &parameters) {
  ...
  bool ret = gtk_list_store_iter_is_valid(GTK_LIST_STORE(model), &iter) != 0;
...
void GtkListStore_::swap(Php::Parameters &parameters) noexcept {
  GtkTreeIter a;
  if (!parameters.empty()) {
...
void GtkListStore_::move_before(Php::Parameters &parameters) {
  ...
  if (parameters.size() > 1) {
    ...
    GtkTreeIter_ *phpgtk_position =
        (GtkTreeIter_ *)object_position.implementation();
...
void GtkListStore_::set_sort_func(Php::Parameters &parameters) {
  ...
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(model), sort_column_id,
                                  set_sort_func_callback,
                                  (gpointer)callback_object, nullptr);
}

gint GtkListStore_::set_sort_func_callback(GtkTreeModel *model, GtkTreeIter *a,
  ...
  if (callback_object->user_parameters.size() < 2 ||
      callback_object->user_parameters[1].isNull()) {
...
void GtkListStore_::set_sort_column_id(Php::Parameters &parameters) {
  ...
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(model), sort_column_id,
                                       order);
}
...
Php::Value GtkListStore_::get_iter_first(Php::Parameters &parameters) {
  ...
  bool ret = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(model), &iter) != 0;
