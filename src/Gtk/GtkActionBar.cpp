...
Php::Value GtkActionBar_::get_center_widget() {
  gpointer *ret =
      (gpointer *)gtk_action_bar_get_center_widget(GTK_ACTION_BAR(instance));
...
void GtkActionBar_::set_center_widget(Php::Parameters &parameters) {
  GtkWidget *center_widget = nullptr;
  if (!parameters.empty()) {
    Php::Value object_center_widget = parameters[0];
    GtkWidget_ *phpgtk_center_widget =
        (GtkWidget_ *)object_center_widget.implementation();
