...
Php::Value GtkTreeStore_::remove(Php::Parameters &parameters) {
  ...
  bool ret = gtk_tree_store_remove(GTK_TREE_STORE(model), &iter) != 0;
...
Php::Value GtkTreeStore_::insert_with_values(Php::Parameters &parameters) {
  ...
  // gboolean ret = gtk_tree_store_insert_with_valuesv (GTK_TREE_STORE(model),
  // &iter, &parent, position, columns, values, g_values); if(!ret) { 	return
  // false;
  // }
  ...
  // return Php::Object("GtkTreeIter", return_parsed);
  throw Php::Exception(
      "GtkTreeStore->insert_with_values() not implemented yet");
}
...
Php::Value GtkTreeStore_::append(Php::Parameters &parameters) {
  ...
  if (parameters[0]) {
    ...
    GtkTreeIter_ *phpgtk_parent =
        (GtkTreeIter_ *)object_parent.implementation();
...
Php::Value GtkTreeStore_::is_ancestor(Php::Parameters &parameters) {
  ...
  GtkTreeIter_ *phpgtk_descendant =
      (GtkTreeIter_ *)object_descendant.implementation();
  GtkTreeIter descendant = phpgtk_descendant->get_instance();

  bool ret = gtk_tree_store_is_ancestor(GTK_TREE_STORE(model), &iter,
                                        &descendant) != 0;
...
void GtkTreeStore_::clear() { gtk_tree_store_clear(GTK_TREE_STORE(model)); }
...
void GtkTreeStore_::swap(Php::Parameters &parameters) noexcept {
