...
void GtkWindow_::set_transient_for(Php::Parameters &parameters) {
  ...
  gtk_window_set_transient_for(GTK_WINDOW(instance),
                               GTK_WINDOW(passedWindow->get_instance()));
}
...
Php::Value GtkWindow_::get_transient_for() {
  GtkWindow *returnedWindow =
      gtk_window_get_transient_for(GTK_WINDOW(instance));
...
void GtkWindow_::set_hide_titlebar_when_maximized(Php::Parameters &parameters) {
  gtk_window_set_hide_titlebar_when_maximized(GTK_WINDOW(instance),
                                              parameters[0]);
}
...
void GtkWindow_::add_mnemonic(Php::Parameters &parameters) {
  ...
  gtk_window_add_mnemonic(GTK_WINDOW(instance), keyval,
                          GTK_WIDGET(passedWindow->get_instance()));
}
...
void GtkWindow_::remove_mnemonic(Php::Parameters &parameters) {
  ...
  gtk_window_remove_mnemonic(GTK_WINDOW(instance), keyval,
                             GTK_WIDGET(passedWindow->get_instance()));
}
...
Php::Value GtkWindow_::mnemonic_activate(Php::Parameters &parameters) {
  ...
  return gtk_window_mnemonic_activate(GTK_WINDOW(instance), keyval,
                                      passedCasted);
}
...
void GtkWindow_::set_focus(Php::Parameters &parameters) {
  ...
  gtk_window_set_focus(GTK_WINDOW(instance),
                       GTK_WIDGET(passedWidget->get_instance()));
}
...
void GtkWindow_::set_default(Php::Parameters &parameters) {
  ...
  gtk_window_set_default(GTK_WINDOW(instance),
                         GTK_WIDGET(passedWidget->get_instance()));
}
...
Php::Value GtkWindow_::get_default_widget() {
  gpointer *ret =
      (gpointer *)gtk_window_get_default_widget(GTK_WINDOW(instance));
 ...
 */
void GtkWindow_::close() { gtk_window_close(GTK_WINDOW(instance)); }

/**
 * Asks to iconify (i.e. minimize) the specified window
 */
void GtkWindow_::iconify() { gtk_window_iconify(GTK_WINDOW(instance)); }

/**
 * Asks to deiconify (i.e. unminimize) the specified window
 */
void GtkWindow_::deiconify() { gtk_window_deiconify(GTK_WINDOW(instance)); }

/**
 * Asks to stick window , which means that it will appear on all user desktops
 */
void GtkWindow_::stick() { gtk_window_stick(GTK_WINDOW(instance)); }

/**
 * Asks to unstick window , which means that it will appear on only one of the
 * user’s desktops.
 */
void GtkWindow_::unstick() { gtk_window_unstick(GTK_WINDOW(instance)); }

/**
 * Asks to maximize window , so that it becomes full-screen.
 */
void GtkWindow_::maximize() { gtk_window_maximize(GTK_WINDOW(instance)); }

/**
 * Asks to unmaximize window
 */
void GtkWindow_::unmaximize() { gtk_window_unmaximize(GTK_WINDOW(instance)); }

/**
 * Asks to place window in the fullscreen state.
 */
void GtkWindow_::fullscreen() { gtk_window_fullscreen(GTK_WINDOW(instance)); }
...
Php::Value GtkWindow_::set_icon_from_file(Php::Parameters &parameters) {
  ...
  return gtk_window_set_icon_from_file(GTK_WINDOW(instance), filename.c_str(),
                                       nullptr);
}
 ...
 */
void GtkWindow_::present() { gtk_window_present(GTK_WINDOW(instance)); }
...
/*
 * Sets an icon to be used as fallback for windows that haven't had
 * gtk_window_set_default_icon_list() called on them from a file on disk
 *
 * @todo not tested
 */
Php::Value GtkWindow_::set_default_icon_from_file(Php::Parameters &parameters) {
  ...
  return gtk_window_set_default_icon_from_file(filename.c_str(), nullptr);
}

/**
 * Sets an icon to be used as fallback for windows that haven't had
 * gtk_window_set_default_icon_list() called on them from a named themed icon,
 * see gtk_window_set_default_icon_name().
 *
 * @todo not tested
 */
...
void GtkWindow_::begin_move_drag(Php::Parameters &parameters) {
  if (parameters.size() != 4) {
    throw Php::Exception("begin_move_drag expects 4 parameters: button, "
  ...
  gtk_window_begin_move_drag(GTK_WINDOW(instance), button, root_x, root_y,
                             timestamp);
}
...
void GtkWindow_::begin_resize_drag(Php::Parameters &parameters) {
  if (parameters.size() != 5) {
    throw Php::Exception("begin_resize_drag expects 5 parameters: edge, "
  ...
  gtk_window_begin_resize_drag(GTK_WINDOW(instance), edge, button, root_x,
                               root_y, timestamp);
}
...
void GtkWindow_::begin_resize_drag(Php::Parameters &parameters) {
  if (parameters.size() < 5) {
    throw Php::Exception(
        "Expected 5 parameters: edge, button, root_x, root_y, timestamp");
  }
  ...
  gtk_window_begin_resize_drag(GTK_WINDOW(instance), (GdkWindowEdge)edge,
                               button, root_x, root_y, timestamp);
}
...
void GtkWindow_::add_accel_group(Php::Parameters &parameters) {
  if (parameters.empty()) {
    throw Php::Exception("Expected 1 parameter: accel_group");
  }
