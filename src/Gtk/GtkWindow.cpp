...
/**
 * Requests that the window is closed, similar to what happens when a window manager close button is
 * clicked.
 */
...
/**
 * By setting the type hint for the window, you allow the window manager to decorate and handle the
 * window in a way which is suitable to the function of the window in your application.
 */
...
/**
 * Sets an icon to be used as fallback for windows that haven't had gtk_window_set_icon() called on
 * them from a pixbuf.
 *
 * @todo not tested
 */
...
void GtkWindow_::begin_resize_drag(Php::Parameters &parameters) {
  if (parameters.size() != 5) {
    throw Php::Exception(
        "begin_resize_drag expects 5 parameters: edge, button, root_x, root_y, timestamp");
  }
