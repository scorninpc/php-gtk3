...
Php::Value GtkTextTagTable_::add(Php::Parameters &parameters) {
  ...
  bool ret = gtk_text_tag_table_add(GTK_TEXT_TAG_TABLE(instance), tag) != 0;
...
Php::Value GtkTextTagTable_::lookup(Php::Parameters &parameters) {
  ...
  GtkTextTag *ret =
      gtk_text_tag_table_lookup(GTK_TEXT_TAG_TABLE(instance), name);
...
Php::Value GtkTextTagTable_::foreach (Php::Parameters &parameters) {
  ...
  // 	GtkTextTagTableForeach_ *phpgtk_func = (GtkTextTagTableForeach_
  // *)object_func.implementation(); 	func = (phpgtk_func->get_instance());
  // }

  // gpointer data = (gpointer)parameters[1];

  // void ret = gtk_text_tag_table_foreach (GTK_TEXT_TAG_TABLE(instance), func,
  // data);
