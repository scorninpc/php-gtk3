 ...
 *
 */
void Gtk_::main() { gtk_main(); }

/**
 *
 */
void Gtk_::main_quit() { gtk_main_quit(); }
...
gint Gtk_::timeout_add_callback(gpointer data) {
  ...
  Php::Value ret;
  try {
    ret = Php::call("call_user_func_array", callback_object->callback_name,
                    internal_parameters);
...
Php::Value Gtk_::source_remove(Php::Parameters &parameters) {
  ...
  bool ret = g_source_remove(tag) != 0;
...
Php::Value Gtk_::is_destroyed(Php::Parameters &parameters) {
  ...
  GSource *source = g_main_context_find_source_by_id(
      nullptr, tag); // Get source from default main context
...
Php::Value Gtk_::events_pending() { return gtk_events_pending(); }

Php::Value Gtk_::main_do_event(Php::Parameters &parameters) {
  if (parameters.empty()) {
    throw Php::Exception(
        "Gtk::main_do_event requires a GdkEvent as a parameter.");
  }
  ...
  GdkEvent_ *phpgtk_event =
      dynamic_cast<GdkEvent_ *>(object_event.implementation());
  if (phpgtk_event == nullptr) {
...
Php::Value Gtk_::main_iteration() { return gtk_main_iteration(); }
...
void Gtk_::init() { gtk_init(nullptr, nullptr); }
