...
static void suppress_gfileinfo_warnings(const gchar *log_domain,
  ...
  if (g_str_has_prefix(message, "GFileInfo created without standard::size") ||
      ...
      (g_strstr_len(message, -1, "g_file_info_get_size") != nullptr) ||
      (g_strstr_len(message, -1, "g_file_info_get_file_type") != nullptr)) {
...
void GtkAppChooserDialog_::__construc(Php::Parameters &parameters) {
  ...
  guint handler_id = g_log_set_handler("GLib-GIO", G_LOG_LEVEL_CRITICAL,
                                       suppress_gfileinfo_warnings, nullptr);
...
Php::Value
GtkAppChooserDialog_::new_for_content_type(Php::Parameters &parameters) {
  Php::Value object_parent = parameters[0];
  GtkWindow *parent = nullptr;
  if (object_parent.instanceOf("GtkWindow")) {
    GtkWindow_ *phpgtk_parent = (GtkWindow_ *)object_parent.implementation();
...
Php::Value GtkAppChooserDialog_::get_widget() {
  gpointer *ret = (gpointer *)gtk_app_chooser_dialog_get_widget(
      GTK_APP_CHOOSER_DIALOG(instance));
...
Php::Value GtkAppChooserDialog_::get_heading() {
  std::string ret =
      gtk_app_chooser_dialog_get_heading(GTK_APP_CHOOSER_DIALOG(instance));
