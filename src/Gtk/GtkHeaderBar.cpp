...
Php::Value GtkHeaderBar_::get_has_subtitle() {
  bool ret = gtk_header_bar_get_has_subtitle(GTK_HEADER_BAR(instance)) != 0;

  return ret;
}

void GtkHeaderBar_::set_custom_title(Php::Parameters &parameters) {
  GtkWidget *title_widget = nullptr;
  if (!parameters.empty()) {
    Php::Value object_title_widget = parameters[0];
    GtkWidget_ *phpgtk_title_widget =
        (GtkWidget_ *)object_title_widget.implementation();
...
Php::Value GtkHeaderBar_::get_custom_title() {
  gpointer *ret =
      (gpointer *)gtk_header_bar_get_custom_title(GTK_HEADER_BAR(instance));
...
Php::Value GtkHeaderBar_::get_show_close_button() {
  bool ret =
      gtk_header_bar_get_show_close_button(GTK_HEADER_BAR(instance)) != 0;
...
Php::Value GtkHeaderBar_::get_decoration_layout() {
  std::string ret =
      gtk_header_bar_get_decoration_layout(GTK_HEADER_BAR(instance));
