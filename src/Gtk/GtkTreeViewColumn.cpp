...
void GtkTreeViewColumn_::__construct(Php::Parameters &parameters) {
  ...
  // if (!object.instanceOf("GtkCellRenderer")) throw Php::Exception("parameter expect
  // GtkCellRenderer instance");
  ...
  instance = (gpointer *)gtk_tree_view_column_new_with_attributes(
      title.c_str(), GTK_CELL_RENDERER(passedRenderer->get_instance()), t_column.c_str(), n_column,
      NULL);
}
...
void GtkTreeViewColumn_::pack_start(Php::Parameters &parameters) {
  ...
  gtk_tree_view_column_pack_start(GTK_TREE_VIEW_COLUMN(instance),
...
void GtkTreeViewColumn_::pack_end(Php::Parameters &parameters) {
  ...
  gtk_tree_view_column_pack_end(GTK_TREE_VIEW_COLUMN(instance),
                                GTK_CELL_RENDERER(passedRenderer->get_instance()), parameters[1]);
}

/**
 * If resizable is TRUE, then the user can explicitly resize the column by grabbing the outer edge
...
/**
 * Returns the GtkWidget in the button on the column header. If a custom widget has not been set
...
/**
 * Sets the logical sort_column_id that this column sorts on when this column is selected for
 * sorting. Doing so makes the column header clickable.
 */
...
/**
 * Call this function with a setting of TRUE to display an arrow in the header button indicating the
 * column is sorted.
 */
