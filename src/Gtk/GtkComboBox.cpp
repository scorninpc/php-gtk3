...
Php::Value GtkComboBox_::new_with_area(Php::Parameters &parameters) {
  ...
  // 	GtkCellArea_ *phpgtk_area = (GtkCellArea_
  // *)object_area.implementation(); 	area =
  // GTK_CELL_AREA(phpgtk_area->get_instance());
  // }
...
Php::Value GtkComboBox_::get_active_iter() {
  GtkTreeIter iter;
  bool ret = gtk_combo_box_get_active_iter(GTK_COMBO_BOX(instance), &iter) != 0;
  if (!ret) {
    return (bool)ret;
  }
...
void GtkComboBox_::popup() { gtk_combo_box_popup(GTK_COMBO_BOX(instance)); }

void GtkComboBox_::popdown() { gtk_combo_box_popdown(GTK_COMBO_BOX(instance)); }

Php::Value GtkComboBox_::get_popup_accessible() {
  // AtkObject ret = gtk_combo_box_get_popup_accessible
  // (GTK_COMBO_BOX(instance));
...
Php::Value GtkComboBox_::get_row_separator_func() {
  // std::string ret = gtk_combo_box_get_row_separator_func
  // (GTK_COMBO_BOX(instance));
...
void GtkComboBox_::set_row_separator_func(Php::Parameters &parameters) {
  ...
  // gtk_combo_box_set_row_separator_func (GTK_COMBO_BOX(instance), func, data,
  // destroy);
...
Php::Value GtkComboBox_::get_button_sensitivity() {
  GtkSensitivityType ret =
      gtk_combo_box_get_button_sensitivity(GTK_COMBO_BOX(instance));

  return ret;
}

Php::Value GtkComboBox_::get_has_entry() {
  bool ret = gtk_combo_box_get_has_entry(GTK_COMBO_BOX(instance)) != 0;
...
Php::Value GtkComboBox_::get_popup_fixed_width() {
  bool ret = gtk_combo_box_get_popup_fixed_width(GTK_COMBO_BOX(instance)) != 0;
...
void GtkComboBox_::pack_start(Php::Parameters &parameters) {
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(instance),
                             GTK_CELL_RENDERER(passedRenderer->get_instance()),
                             expand);
}
...
void GtkComboBox_::pack_end(Php::Parameters &parameters) {
  ...
  gtk_cell_layout_pack_end(GTK_CELL_LAYOUT(instance),
                           GTK_CELL_RENDERER(passedRenderer->get_instance()),
                           expand);
}
...
void GtkComboBox_::reorder(Php::Parameters &parameters) {
  ...
  gtk_cell_layout_reorder(GTK_CELL_LAYOUT(instance),
                          GTK_CELL_RENDERER(passedRenderer->get_instance()),
                          position);
}
...
void GtkComboBox_::add_attribute(Php::Parameters &parameters) {
  ...
  gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(instance),
      GTK_CELL_RENDERER(passedRenderer->get_instance()), atribute.c_str(),
      column);
}