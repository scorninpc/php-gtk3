...
void GtkEntry_::set_completion(Php::Parameters &parameters) {
  GtkEntryCompletion *completion = nullptr;
  if (!parameters.empty()) {
...
/**
 * Changes the max size request of the entry to be about the right size for
 * n_chars characters.
 */
...
Php::Value GtkEntry_::get_selection_bounds() {
  ...
  bool ret = gtk_editable_get_selection_bounds(GTK_EDITABLE(instance),
                                               &start_pos, &end_pos) != 0;
...
/**
 * Inserts new_text_length bytes of new_text into the contents of the widget, at
 * position position.
 *
...
void GtkEntry_::insert_text(Php::Parameters &parameters) {
  ...
  gtk_editable_insert_text(GTK_EDITABLE(instance), new_text.c_str(),
