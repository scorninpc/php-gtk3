 ...
 */
GtkTreeModel *GtkTreeModel_::get_model() const { return model; }
...
Php::Value GtkTreeModel_::get_iter(Php::Parameters &parameters) const {
  ...
  bool ret = gtk_tree_model_get_iter(GTK_TREE_MODEL(model), &iter, path) != 0;
  if (!ret) {
    return false;
  }
...
Php::Value GtkTreeModel_::get_value(Php::Parameters &parameters) const {
  // Cast iter back
...
Php::Value GtkTreeModel_::get_path(Php::Parameters &parameters) const {
  // Cast iter back
  Php::Value object_iter = parameters[0];
  GtkTreeIter_ *phpgtk_iter = (GtkTreeIter_ *)object_iter.implementation();
  GtkTreeIter iter = phpgtk_iter->get_instance();
...
Php::Value
GtkTreeModel_::get_iter_from_string(Php::Parameters &parameters) const {
  std::string param_path = parameters[0];
