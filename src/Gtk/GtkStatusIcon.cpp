...
Php::Value GtkStatusIcon_::get_gicon() {
  ...
  // The GIcon_ wrapper takes its own reference via set_instance(), so it can safely outlive the
  // status icon. However, its contents may become stale if the status icon's GIcon is changed
  // later.
...
void GtkStatusIcon_::set_blinking(Php::Parameters &parameters) {
  ...
  throw Php::Exception(
      "GtkStatusIcon::set_blinking is not available in GTK 3. This function was removed in GTK "
      "3.14. Consider using a timer to alternate icons for a blinking effect.");
}

Php::Value GtkStatusIcon_::get_blinking() {
  throw Php::Exception(
      "GtkStatusIcon::get_blinking is not available in GTK 3. This function was removed in GTK "
      "3.14.");
}
