...
// GDK_IS_WAYLAND_DISPLAY is available when compiled with Wayland support
  ...
  return ((backend != nullptr) && g_str_has_prefix(backend, "GdkWayland"));
#else
  return false;
#endif
}
...
static void warn_if_wayland() {
  ...
  if (!warning_shown && is_wayland_backend()) {
    warning_shown = true;
    Php::warning
        << "GtkStatusIcon is deprecated and does not work on Wayland. "
        << "The system tray icon will not be visible. "
        << "Possible workarounds: "
        << "(1) Set GDK_BACKEND=x11 to force X11 compatibility mode, or "
        << "(2) Use a desktop environment that supports XEmbed system tray "
           "protocol."
        << std::flush;
  }
}
...
void GtkStatusIcon_::set_from_pixbuf(Php::Parameters &parameters) {
  ...
  // GTK internally calls gtk_widget_get_scale_factor on GtkStatusIcon during
  // icon loading, which triggers an assertion since GtkStatusIcon is not a
  // GtkWidget
...
{
        // GtkImageType ret =
gtk_status_icon_get_storage_type(GTK_STATUS_ICON(instance));
...
Php::Value GtkStatusIcon_::get_gicon() {
  ...
  // Note: gtk_status_icon_get_gicon() returns a borrowed reference owned by the
  // status icon. The GIcon_ wrapper takes its own reference via set_instance(),
  // so it can safely outlive the status icon. However, its contents may become
  // stale if the status icon's GIcon is changed later.
...
Php::Value GtkStatusIcon_::get_visible() {
  bool ret = gtk_status_icon_get_visible(GTK_STATUS_ICON(instance)) != 0;

  return ret;
}

Php::Value GtkStatusIcon_::is_embedded() {
  bool ret = gtk_status_icon_is_embedded(GTK_STATUS_ICON(instance)) != 0;

  return ret;
}

Php::Value GtkStatusIcon_::get_has_tooltip() {
  bool ret = gtk_status_icon_get_has_tooltip(GTK_STATUS_ICON(instance)) != 0;
...
{
        // guint32 ret =
gtk_status_icon_get_x11_window_id(GTK_STATUS_ICON(instance));
...
Php::Value GtkStatusIcon_::position_menu(Php::Parameters &parameters) {
  ...
  // Use stack variables and pass their addresses to
  // gtk_status_icon_position_menu
  gint x = 0;
  gint y = 0;
  gboolean push_in = FALSE;

  gtk_status_icon_position_menu(menu, &x, &y, &push_in,
                                GTK_STATUS_ICON(instance));
...
void GtkStatusIcon_::set_blinking(Php::Parameters &parameters) {
  ...
  throw Php::Exception("GtkStatusIcon::set_blinking is not available in GTK 3. "
                       "This function was removed in GTK 3.14. Consider using "
                       "a timer to alternate icons for a blinking effect.");
}

Php::Value GtkStatusIcon_::get_blinking() {
  throw Php::Exception("GtkStatusIcon::get_blinking is not available in GTK 3. "
                       "This function was removed in GTK 3.14.");
}
