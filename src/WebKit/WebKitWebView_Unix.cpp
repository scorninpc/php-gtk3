 ...
 * Constructor
 */
WebKitWebView_::WebKitWebView_() : user_content_manager(nullptr) = default;
...
void WebKitWebView_::__construct() {
  ...
  user_content_manager =
      webkit_web_view_get_user_content_manager(WEBKIT_WEB_VIEW(instance));
...
void WebKitWebView_::run_javascript(Php::Parameters &parameters) {
  ...
  webkit_web_view_evaluate_javascript(WEBKIT_WEB_VIEW(instance), script, -1,
...
static void script_message_received_cb(WebKitUserContentManager *manager,
                                       const WebKitJavascriptResult *js_result,
                                       gpointer user_data) {
...
void WebKitWebView_::register_script_message_handler(
  ...
  if (parameters.size() > 1 && parameters[1].isCallable()) {
    ...
    gulong handler_id = 0 = g_signal_connect_data(
        user_content_manager, signal_name.c_str(),
        G_CALLBACK(script_message_received_cb), data,
...
void WebKitWebView_::enable_developer_extras() {
  ...
  WebKitSettings *settings =
      webkit_web_view_get_settings(WEBKIT_WEB_VIEW(instance));
...
void WebKitWebView_::set_user_data_folder(Php::Parameters &params) {
  ...
  // by WebKitWebsiteDataManager and is set per WebKitWebContext, not per
  // WebView. This method is a no-op on Unix platforms for API compatibility
  // with Windows.
  //
  // Users can configure this at the WebKitWebContext level if needed.
  g_warning("WebView: set_user_data_folder() is only supported on Windows with "
            "WebView2. "
