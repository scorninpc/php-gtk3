#ifndef _WIN32
// Unix/Linux/macOS implementation using WebKit2GTK
...
WebKitWebView_::~WebKitWebView_() {
    ...
    if (user_content_manager != nullptr && G_IS_OBJECT(user_content_manager)) {
        for (gulong handler_id : signal_handler_ids) {
            if (g_signal_handler_is_connected(user_content_manager, handler_id)) {
...
void WebKitWebView_::reload() { webkit_web_view_reload(WEBKIT_WEB_VIEW(instance)); }

void WebKitWebView_::stop_loading() { webkit_web_view_stop_loading(WEBKIT_WEB_VIEW(instance)); }
...
void WebKitWebView_::go_back() { webkit_web_view_go_back(WEBKIT_WEB_VIEW(instance)); }
...
void WebKitWebView_::go_forward() { webkit_web_view_go_forward(WEBKIT_WEB_VIEW(instance)); }
...
void WebKitWebView_::run_javascript(Php::Parameters &parameters) {
    ...
    webkit_web_view_evaluate_javascript(WEBKIT_WEB_VIEW(instance), script, -1, nullptr, nullptr, nullptr, nullptr,
                                        nullptr);
}
...
static void script_message_received_cb(WebKitUserContentManager *manager, WebKitJavascriptResult *js_result,
                                       gpointer user_data) {
...
void WebKitWebView_::register_script_message_handler(Php::Parameters &parameters) {
    ...
    if (parameters.size() > 1 && parameters[1].isCallable()) {
        ...
        gulong handler_id = g_signal_connect_data(
            user_content_manager, signal_name.c_str(), G_CALLBACK(script_message_received_cb), data,
            [](gpointer user_data, GClosure *closure) { delete (ScriptMessageData *)user_data; }, (GConnectFlags)0);
...
void WebKitWebView_::set_user_data_folder(Php::Parameters &params) {
    // On Unix/Linux/macOS with WebKit2GTK, the user data folder is managed
    // by WebKitWebsiteDataManager and is set per WebKitWebContext, not per WebView.
    // This method is a no-op on Unix platforms for API compatibility with Windows.
    //
    // Users can configure this at the WebKitWebContext level if needed.
    g_warning(
        "WebView: set_user_data_folder() is only supported on Windows with WebView2. "
        "On Unix/Linux/macOS, configure WebKitWebsiteDataManager instead.");
}
