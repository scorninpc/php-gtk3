...
struct WebView2State {
  ...
  std::vector<std::string>
      pending_scripts;          // Scripts to run once WebView is initialized
  std::string user_data_folder; // Custom user data folder path
  std::wstring
      user_data_folder_wide; // Wide string version for async WebView2 creation
  bool is_loading;
  std::map<std::string, Php::Value> message_handlers;
  gulong realize_signal_handler_id; // Signal handler ID for cleanup

  WebView2State()
      : hwnd(nullptr), is_loading(false), realize_signal_handler_id(0) {}
};
...
// Get native Windows HWND
  ...
  std::string folder_path =
      state->user_data_folder.empty() ? "./tmp" : state->user_data_folder;

  int required_size =
      MultiByteToWideChar(CP_UTF8, 0, folder_path.c_str(), -1, nullptr, 0);
  if (required_size > 0) {
    state->user_data_folder_wide.resize(required_size);
    MultiByteToWideChar(CP_UTF8, 0, folder_path.c_str(), -1,
                        &state->user_data_folder_wide[0], required_size);
  ...
  CreateCoreWebView2EnvironmentWithOptions(
      ...
      Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
          [state](HRESULT result, ICoreWebView2Environment *env) -> HRESULT {
            if (FAILED(result)) {
              g_warning("WebView2: Failed to create environment: 0x%lx",
                        result);
              return result;
            }
            ...
            env->CreateCoreWebView2Controller(
                ...
                Callback<
                    ...
                    [state](HRESULT result,
                      ...
                      if (FAILED(result)) {
                        g_warning(
                            "WebView2: Failed to create controller: 0x%lx",
                            result);
                        return result;
                      }
                              ...
                              [state](ICoreWebView2 *sender,
                                ...
                                if (uri) {
                                  int required_size = WideCharToMultiByte(
                                      CP_UTF8, 0, uri, -1, nullptr, 0, nullptr,
                                      nullptr);
                                  if (required_size > 0) {
                                    std::vector<char> buffer(required_size);
                                    WideCharToMultiByte(
                                        CP_UTF8, 0, uri, -1, buffer.data(),
                      ...
                      state->webview->add_NavigationCompleted(
                          Callback<
                              ...
                              [state](ICoreWebView2 *sender,
                                ...
                                // Re-inject message handler compatibility layer
                                // after navigation This is necessary because
                                // page navigation clears all injected scripts
                                for (const auto &handler :
                                     state->message_handlers) {
                                  const std::string &handlerName =
                                      handler.first;
                                  if (!is_valid_handler_name(handlerName)) {
                                    g_warning("WebView2: Skipping re-injection "
                                              "of invalid handler name: %s",
                                              handlerName.c_str());
                                  ...
                                  int required_size = MultiByteToWideChar(
                                      CP_UTF8, 0, inject_script.c_str(), -1,
                                      nullptr, 0);
                                  if (required_size > 0) {
                                    std::vector<wchar_t> w_script(
                                        required_size);
                                    MultiByteToWideChar(
                                        CP_UTF8, 0, inject_script.c_str(), -1,
                                        w_script.data(), required_size);
                      ...
                      state->webview->add_WebMessageReceived(
                          Callback<ICoreWebView2WebMessageReceivedEventHandler>(
                              [state](ICoreWebView2 *sender,
                                ...
                                if (message) {
                                  int required_size = WideCharToMultiByte(
                                      CP_UTF8, 0, message, -1, nullptr, 0,
                                      nullptr, nullptr);
                                  if (required_size > 0) {
                                    std::vector<char> buffer(required_size);
                                    WideCharToMultiByte(
                                        CP_UTF8, 0, message, -1, buffer.data(),
                                        required_size, nullptr, nullptr);

                                    // Parse message format:
                                    // "handlerName:jsonData"
                                    std::string msg_str(buffer.data());
                                    size_t colon_pos = msg_str.find(':');
                                    if (colon_pos != std::string::npos) {
                                      std::string handler_name =
                                          msg_str.substr(0, colon_pos);
                                      std::string json_data =
                                          msg_str.substr(colon_pos + 1);

                                      // Remove JSON quotes if present (e.g.,
                                      // "\"message\"" -> "message")
                                      ...
                                      auto it = state->message_handlers.find(
                                          handler_name);
                                      if (it != state->message_handlers.end() &&
                                          it->second.isCallable()) {
                                        try {
                                          it->second(data);
                                        } catch (...) {
                                          g_warning("WebView2: Exception in "
                                                    "message handler");
                                        }
                                      }
                                    }
                      ...
                      if (!state->pending_uri.empty()) {
                        int required_size = MultiByteToWideChar(
                            CP_UTF8, 0, state->pending_uri.c_str(), -1, nullptr,
                            0);
                        if (required_size > 0) {
                          std::vector<wchar_t> w_uri(required_size);
                          MultiByteToWideChar(CP_UTF8, 0,
                                              state->pending_uri.c_str(), -1,
                                              w_uri.data(), required_size);
                          state->webview->Navigate(w_uri.data());
                          state->pending_uri.clear();
                        }
                      } else if (!state->pending_html.empty()) {
                        int required_size = MultiByteToWideChar(
                            CP_UTF8, 0, state->pending_html.c_str(), -1,
                            nullptr, 0);
                        if (required_size > 0) {
                          std::vector<wchar_t> w_html(required_size);
                          MultiByteToWideChar(CP_UTF8, 0,
                                              state->pending_html.c_str(), -1,
                                              w_html.data(), required_size);
                      ...
                      if (!state->pending_scripts.empty()) {
                        for (const auto &script : state->pending_scripts) {
                          int required_size = MultiByteToWideChar(
                              CP_UTF8, 0, script.c_str(), -1, nullptr, 0);
                          if (required_size > 0) {
                            std::vector<wchar_t> w_script(required_size);
                            MultiByteToWideChar(CP_UTF8, 0, script.c_str(), -1,
                                                w_script.data(), required_size);
                            state->webview->ExecuteScript(
                                w_script.data(),
                                Callback<
...
WebKitWebView_::~WebKitWebView_() {
  ...
  if (state) {
    ...
    if (state->realize_signal_handler_id != 0 && instance != nullptr &&
        GTK_IS_WIDGET(instance)) {
      g_signal_handler_disconnect(GTK_WIDGET(instance),
                                  state->realize_signal_handler_id);
...
void WebKitWebView_::__construct() {
  ...
  state->realize_signal_handler_id = g_signal_connect(
      GTK_WIDGET(instance), "realize", G_CALLBACK(on_widget_realized), state);
}
...
void WebKitWebView_::load_uri(Php::Parameters &parameters) {
  ...
  int required_size =
      MultiByteToWideChar(CP_UTF8, 0, s_uri.c_str(), -1, nullptr, 0);
  if (required_size > 0) {
    std::vector<wchar_t> w_uri(required_size);
    MultiByteToWideChar(CP_UTF8, 0, s_uri.c_str(), -1, w_uri.data(),
                        required_size);
    state->webview->Navigate(w_uri.data());
  } else {
    g_warning("WebView2: Failed to convert URI to wide string: %s",
              s_uri.c_str());
    return;
  }
}
...
void WebKitWebView_::load_html(Php::Parameters &parameters) {
  ...
  // Convert UTF-8 to wide string
  int required_size =
      MultiByteToWideChar(CP_UTF8, 0, s_content.c_str(), -1, nullptr, 0);
  if (required_size > 0) {
    std::vector<wchar_t> w_content(required_size);
    MultiByteToWideChar(CP_UTF8, 0, s_content.c_str(), -1, w_content.data(),
                        required_size);
...
void WebKitWebView_::run_javascript(Php::Parameters &parameters) {
  if (parameters.size() == 0) {
    throw Php::Exception(
        "run_javascript() expects at least 1 parameter, 0 given");
  }
  ...
  // Convert UTF-8 to wide string
  int required_size =
      MultiByteToWideChar(CP_UTF8, 0, s_script.c_str(), -1, nullptr, 0);
  if (required_size > 0) {
    std::vector<wchar_t> w_script(required_size);
    MultiByteToWideChar(CP_UTF8, 0, s_script.c_str(), -1, w_script.data(),
                        required_size);
...
void WebKitWebView_::register_script_message_handler(
    Php::Parameters &parameters) {
  if (parameters.size() == 0) {
    throw Php::Exception("register_script_message_handler() expects at least 1 "
                         "parameter, 0 given");
  }
  ...
  if (!is_valid_handler_name(s_name)) {
    throw Php::Exception(
        "register_script_message_handler(): Invalid handler name. Only "
        "alphanumeric characters and underscores are "
        "allowed.");
  }
  ...
  // This makes window.webkit.messageHandlers.handlerName.postMessage() work on
  // WebView2
  if (state->webview) {
    // Use bracket notation for safe injection
    std::string inject_script = "(function() {"
                                "  if (!window.webkit) window.webkit = {};"
                                "  if (!window.webkit.messageHandlers) "
                                "window.webkit.messageHandlers = {};"
                                "  window.webkit.messageHandlers[" +
    ...
    if (required_size > 0) {
      std::vector<wchar_t> w_script(required_size);
      MultiByteToWideChar(CP_UTF8, 0, inject_script.c_str(), -1,
                          w_script.data(), required_size);
...
void WebKitWebView_::set_user_data_folder(Php::Parameters &params) {
  if (params.size() < 1) {
    throw Php::Exception(
        "set_user_data_folder() requires 1 parameter: folder path");
  }
  ...
  if (state->webview) {
    g_warning("WebView2: set_user_data_folder() must be called before the "
              "widget is realized. Setting will take effect "
