...
// Get native Windows HWND
  ...
  if (required_size > 0) {
    ...
    MultiByteToWideChar(CP_UTF8, 0, folder_path.c_str(), -1, &state->user_data_folder_wide[0],
                        required_size);
  ...
  CreateCoreWebView2EnvironmentWithOptions(
      ...
      Callback<
          ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>([state](
                                                                          HRESULT result,
            ...
            Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                [state](HRESULT result, ICoreWebView2Controller *controller) -> HRESULT {
                  ...
                  state->webview->add_NavigationStarting(
                      Callback<ICoreWebView2NavigationStartingEventHandler>(
                          [state](ICoreWebView2 *sender,
                            ...
                            args->get_Uri(&uri);
                            if (uri) {
                              int required_size = WideCharToMultiByte(CP_UTF8, 0, uri, -1, nullptr,
                                                                      0, nullptr, nullptr);
                              if (required_size > 0) {
                                std::vector<char> buffer(required_size);
                                WideCharToMultiByte(CP_UTF8, 0, uri, -1, buffer.data(),
                  ...
                  state->webview->add_NavigationCompleted(
                      Callback<ICoreWebView2NavigationCompletedEventHandler>(
                          [state](ICoreWebView2 *sender,
                            ...
                            if (title) {
                              int required_size = WideCharToMultiByte(CP_UTF8, 0, title, -1,
                                                                      nullptr, 0, nullptr, nullptr);
                              if (required_size > 0) {
                                std::vector<char> buffer(required_size);
                                WideCharToMultiByte(CP_UTF8, 0, title, -1, buffer.data(),
                                                    required_size, nullptr, nullptr);
                            ...
                            for (const auto &handler : state->message_handlers) {
                              ...
                              if (!is_valid_handler_name(handlerName)) {
                                g_warning(
                                    "WebView2: Skipping re-injection of invalid handler name: %s",
                                    handlerName.c_str());
                              ...
                              int required_size = MultiByteToWideChar(
                                  CP_UTF8, 0, inject_script.c_str(), -1, nullptr, 0);
                              if (required_size > 0) {
                                ...
                                state->webview->ExecuteScript(
                                    ...
                                    Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
                                        [](HRESULT errorCode,
                                           LPCWSTR resultObjectAsJson) -> HRESULT { return S_OK; })
                                        .Get());
                              }
                            }
                  ...
                  state->webview->add_WebMessageReceived(
                      Callback<ICoreWebView2WebMessageReceivedEventHandler>(
                          [state](ICoreWebView2 *sender,
                            ...
                            if (message) {
                              int required_size = WideCharToMultiByte(CP_UTF8, 0, message, -1,
                                                                      nullptr, 0, nullptr, nullptr);
                              if (required_size > 0) {
                                std::vector<char> buffer(required_size);
                                WideCharToMultiByte(CP_UTF8, 0, message, -1, buffer.data(),
                                                    required_size, nullptr, nullptr);
                                ...
                                if (colon_pos != std::string::npos) {
                                  ...
                                  // Remove JSON quotes if present (e.g., "\"message\"" ->
                                  // "message")
                                  std::string data = json_data;
                                  if (data.length() >= 2 && data.front() == '"' &&
                                      data.back() == '"') {
                  ...
                  if (!state->pending_uri.empty()) {
                    int required_size =
                        MultiByteToWideChar(CP_UTF8, 0, state->pending_uri.c_str(), -1, nullptr, 0);
                    if (required_size > 0) {
                    ...
                    int required_size = MultiByteToWideChar(CP_UTF8, 0, state->pending_html.c_str(),
                                                            -1, nullptr, 0);
                    if (required_size > 0) {
                      std::vector<wchar_t> w_html(required_size);
                      MultiByteToWideChar(CP_UTF8, 0, state->pending_html.c_str(), -1,
                  ...
                  if (!state->pending_scripts.empty()) {
                    for (const auto &script : state->pending_scripts) {
                      int required_size =
                          MultiByteToWideChar(CP_UTF8, 0, script.c_str(), -1, nullptr, 0);
                      if (required_size > 0) {
                        std::vector<wchar_t> w_script(required_size);
                        MultiByteToWideChar(CP_UTF8, 0, script.c_str(), -1, w_script.data(),
                                            required_size);
                        state->webview->ExecuteScript(
                            w_script.data(),
                            Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
...
void WebKitWebView_::__construct() {
  ...
  state->realize_signal_handler_id =
      g_signal_connect(GTK_WIDGET(instance), "realize", G_CALLBACK(on_widget_realized), state);
}
...
void WebKitWebView_::register_script_message_handler(Php::Parameters &parameters) {
  ...
  if (!is_valid_handler_name(s_name)) {
    throw Php::Exception(
        "register_script_message_handler(): Invalid handler name. Only alphanumeric characters and "
        "underscores are "
        "allowed.");
  }
...
void WebKitWebView_::set_user_data_folder(Php::Parameters &params) {
  ...
  if (state->webview) {
    g_warning(
        "WebView2: set_user_data_folder() must be called before the widget is realized. Setting "
        "will take effect "
