...
Php::Value Gdk_::test_simulate_button(Php::Parameters &parameters) {
  ...
  // ---
  check =
      phpgtk_check_parameter(parameters, 2, Php::Type::Numeric, FALSE, nullptr);
  gint x = 0;
  if (check) {
    x = (int)parameters[1];
  }
  ...
  // ---
  check =
      phpgtk_check_parameter(parameters, 3, Php::Type::Numeric, FALSE, nullptr);
  gint y = 0;
  if (check) {
    y = (int)parameters[2];
  }
  ...
  // ---
  check =
      phpgtk_check_parameter(parameters, 4, Php::Type::Numeric, FALSE, nullptr);
  gint button = 0;
  if (check) {
  ...
  // ---
  check =
      phpgtk_check_parameter(parameters, 5, Php::Type::Numeric, FALSE, nullptr);
  ...
  // ---
  check =
      phpgtk_check_parameter(parameters, 6, Php::Type::Numeric, FALSE, nullptr);
  ...
  //
  gdk_test_simulate_button(window, x, y, button, modifiers,
                           button_pressrelease);
...
void Gdk_::cairo_set_source_pixbuf(Php::Parameters &parameters) {
  ...
  // Note: Cairo context is a G_TYPE_BOXED that gets wrapped as GdkEvent by
  // GObject callback, even though it's actually a cairo_t*. We extract the raw
  // pointer from GdkEvent::instance.
  if (parameters.empty()) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: Missing cairo_context parameter");
  }
  if (parameters[0].type() != Php::Type::Object) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: cairo_context must be a GdkEvent object");
  }
  ...
  if (cr == nullptr) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: Invalid cairo context (null pointer)");
  }
  ...
  if (parameters[1].type() != Php::Type::Object) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: pixbuf must be a GdkPixbuf object");
  }
  ...
  if (pixbuf == nullptr) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: Invalid pixbuf (null pointer)");
  }
  ...
  if (pixbuf_x_type != Php::Type::Numeric &&
      pixbuf_x_type != Php::Type::Float) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: pixbuf_x must be int or float");
  }
  ...
  if (pixbuf_y_type != Php::Type::Numeric &&
      pixbuf_y_type != Php::Type::Float) {
    throw Php::Exception(
        "cairo_set_source_pixbuf: pixbuf_y must be int or float");
  }
...
void Gdk_::cairo_paint(Php::Parameters &parameters) {
  ...
  if (parameters[0].type() != Php::Type::Object) {
    throw Php::Exception(
        "cairo_paint: cairo_context must be a GdkEvent object");
  }
  if (!parameters[0].instanceOf("GdkEvent")) {
    throw Php::Exception(
        "cairo_paint: cairo_context must be a GdkEvent object");
  ...
  // Call the Cairo function (use :: to call global C function, not the class
  // method)
  ::cairo_paint(cr);
}